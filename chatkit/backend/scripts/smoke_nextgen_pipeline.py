#!/usr/bin/env python3
"""Live smoke test for DS Chat Next-Gen end-to-end pipeline."""
...
from app.threevictors_client import ThreeVictorsClient, ThreeVictorsConfig
    ...
    *,
    ...
    context = AgentContext(
        ...
        request_context={
            ...
            "nextgen_kb": kb,
            "nextgen_threevictors": ThreeVictorsClient(config=ThreeVictorsConfig(environment="3VDEV")),
...
def _build_scenarios() -> list[Scenario]:
    return [
        Scenario(
            ...
            required_output_substrings=("raw_datasets",),
        ),
        Scenario(
        ...
        Scenario(
            ...
            required_tools=("build_investigation_plan",),
            forbidden_tools=("execute_sql_extract",),
        ),
        Scenario(
            ...
            required_output_substrings=("mysql_fallback",),
        ),
        Scenario(
...
def _render_markdown_report(payload: dict[str, Any]) -> str:
    lines: list[str] = []
    lines.append("# DS Chat Next-Gen Smoke Report")
    lines.append("")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run DS Chat Next-Gen E2E smoke pipeline checks.")
    ...
    json_path = (report_dir / f"smoke_nextgen_{run_id}.json").resolve()
    md_path = (report_dir / f"smoke_nextgen_{run_id}.md").resolve()
